...
class expect_empty_sequence_receiver {
  ...
  friend void tag_invoke(ex::P0TBD::set_next_t, expect_empty_sequence_receiver&, ValueSender&&) noexcept {
...
struct expect_empty_sequence_receiver_ex {
  ...
  template <typename ValueSender>
  friend void tag_invoke(ex::P0TBD::set_next_t, expect_empty_sequence_receiver_ex& self, ValueSender&&) noexcept {
...
template <typename ValidFn, bool RuntimeCheck = true>
class expect_sequence_receiver {
  ...
  public:
  explicit expect_sequence_receiver(ValidFn valid)
      : valid_(std::move(valid)) {}
  ~expect_sequence_receiver() { CHECK(ended_ != false); }

  expect_sequence_receiver(expect_sequence_receiver&& other)
      : called_(std::exchange(other.called_, -1))
      , ended_(std::exchange(other.ended_, true))
  ...
  friend auto tag_invoke(ex::P0TBD::set_next_t, expect_sequence_receiver& self, ValueSender&& vs) noexcept {
  ...
  friend empty_env tag_invoke(ex::get_env_t, const expect_sequence_receiver&) noexcept {
    return {};
  }
};

template <typename ValidFn, bool RuntimeCheck = true>
expect_sequence_receiver<ValidFn, RuntimeCheck> make_expect_sequence_receiver(ValidFn&& valid) {
  return expect_sequence_receiver<ValidFn, RuntimeCheck>{std::forward<ValidFn>(valid)};
}
